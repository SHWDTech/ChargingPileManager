...
{
	 	//repairs_img
		public partial class repairs_img
	{
   		     
		public bool Exists(int id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from repairs_img");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(System.Model.repairs_img model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into repairs_img(");			
            strSql.Append("img,pudate,guishu,uploader");
			...
			SqlParameter[] parameters = {
                        ...
                        new SqlParameter("@guishu", SqlDbType.NVarChar,50) ,            
            ...
            parameters[2].Value = model.guishu;                        
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(System.Model.repairs_img model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update repairs_img set ");
			                                                
            strSql.Append(" img = @img , ");                                    
            strSql.Append(" pudate = @pudate , ");                                    
            strSql.Append(" guishu = @guishu , ");                                    
...
SqlParameter[] parameters = {
            ...
            parameters[3].Value = model.guishu;                        
            parameters[4].Value = model.uploader;                        
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from repairs_img ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public System.Model.repairs_img GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id, img, pudate, guishu, uploader  ");			
			strSql.Append("  from repairs_img ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			System.Model.repairs_img model=new System.Model.repairs_img();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
																												...
																												if(ds.Tables[0].Rows[0]["pudate"].ToString()!="")
				{
					model.pudate=DateTime.Parse(ds.Tables[0].Rows[0]["pudate"].ToString());
																																				...
																																				model.guishu= ds.Tables[0].Rows[0]["guishu"].ToString();
																																model.uploader= ds.Tables[0].Rows[0]["uploader"].ToString();
        ...
        public DataSet GetList(string strWhere, params SqlParameter[] parameters)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM repairs_img T");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM repairs_img T");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex, params SqlParameter[] parameters)
		{
			...
			strSql.Append(")AS Row, T.*  from repairs_img T ");
